...
test("should use hook", async () => {
  ...
  expect(typeof result.current[2]).toBe("function");
  await waitFor(() =>
    expect(result.current[0]).toBe("39d11ab1c3c6c9eab3f5b3675f438dbf")
  );
});
...
test("should use initial message", async () => {
  ...
  await waitFor(() =>
    expect(result.current[0]).toBe("c703b927a0c5d56e5a33c4b834053bd4")
  );
});
...
test("should update message", async () => {
  ...
  await waitFor(() =>
    expect(result.current[0]).toBe("dccf67b5b214dc38e729958f6a6be829")
  );
  act(() => result.current[2]("testMessage2"));
  await waitFor(() =>
    expect(result.current[0]).toBe("1ddfe493cdf4176866742a4afa04c840")
  );
});
